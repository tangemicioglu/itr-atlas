...
A curated atlas comparing **information transfer rate (ITR, bits/s)** across all
human-to-machine input modalities: brain-computer interfaces, eye-tracking, speech, typing, Morse,
and more.

The hook is the atlas. The point is **traceability**: every number is sourced and re-derived, and the
site makes it visible when an ITR is being used outside its assumptions. Wolpaw's assumptions
(uniform target prior, symmetric errors, no error-correction cost) rarely hold in real interfaces. See
...
**Goals**
- Compare interfaces on the strictest supportable ITR bound per entry, with every method shown.
- Make every figure auditable: sourced inputs + a fully worked, step-by-step derivation.
- Make caveats visible on the page, not buried in a footnote.
...
**not** primarily about the sensing modality. It comes from the **action space** and the **source
...
entropy) is `fixed-set / ~30 / context-conditioned`. A 2D cursor is `continuous / continuous / none`. A
...
1. **Atlas (home)**: title, a **score-type selector** (Strictest + each method) above the chart
   ...
   controls (search, sort), and a responsive 3-up grid of stat cards. Each card: device photo, name,
...
2. **Entry detail**: self-contained panel with a header (photo, tags, invasiveness), **strictest ITR**
   with full step-by-step derivation, a "method varies by action space" note, any additional full
   derivations, supplementary calculations, and citations.
...
- **Card style**: dense stat block ("trading card").
...
- A shared **compute helper** (TypeScript) implements the *standard* formulas: **Wolpaw bitrate**
  and **confusion-matrix mutual information**. It takes stored `inputs` and emits both the numeric
